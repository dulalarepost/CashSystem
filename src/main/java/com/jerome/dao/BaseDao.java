...
public class BaseDao {
    //使用DataSource连接数据库
    //prepareStatement:存在预编译，不用Statement
    //自动提交，防止手动提交回滚
    ...
    private DataSource getDataSource() {
        if (dataSource == null) {
            synchronized(DataSource.class) {
                if (dataSource == null) {
                    dataSource = new MysqlDataSource();//MySqlc数据源
                    String host = "127.0.0.1";
                    String port = "3306";
                    ((MysqlDataSource) dataSource).setUrl(
                            "jdbc:mysql://" + (host + ":" + port) + "/xiangmu");
                    ((MysqlDataSource) dataSource).setUser("root");
                    ((MysqlDataSource) dataSource).setPassword("p18392501634");
                }
            }
        }
        return dataSource;
